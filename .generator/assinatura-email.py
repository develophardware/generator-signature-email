...
def criar_assinatura(nome, cargo, empresa, telefone, ramal, endereco, imagem_base64=None):
    ...
    <span style="color: #5f5e5e; font-family: 'Source Sans Pro, sans-serif;font-size: 12px;
    font-weight: 600;">{cargo} | {empresa}</span><br>
    <span style="color: #5f5e5e; font-family: 'Source Sans Pro, sans-serif;font-size: 12px;
    font-weight: 600;">Tel: {telefone_formatado} | Ramal: {ramal}</span><br>
    <span style="color: #5f5e5e; font-family: 'Source Sans Pro, sans-serif;font-size: 12px;
    font-weight: 600;">{endereco}</span><br>
    {imagem_html}<br>
    """
    return assinatura
...
def main():
    ...
    if empresa == 'Nova Belenzinho':
        endereco = 'R. São Leopoldo, 811 - Belenzinho, São Paulo - SP, 03055-000'
         ...
         endereco = 'Av. Condessa Elizabeth de Robiano, 2640 - Tatuape, São Paulo - SP, 03074-000'
